...
def draw_rectangles_t(tur, lock):
    lock.acquire()
...
def run_with_threads(tur, log, main_turtle):
    ...
    t1 = threading.Thread(target=draw_squares_t, args=(tur, lock))
    t2 = threading.Thread(target=draw_circles_t, args=(tur, lock))
    t3 = threading.Thread(target=draw_triangles_t, args=(tur, lock))
    t4 = threading.Thread(target=draw_rectangles_t, args=(tur, lock))
